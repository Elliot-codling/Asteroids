...
def initPlayer():
    global player
    player = engine.properties_object("player", f"{file_dir}/assets/textures/spaceship.png", (w/2) - 51 / 2, 518, 45, 51, not performance_mode, 1)

initPlayer()
display += [player]
#preloaded textures
backgroundStarTexture = pygame.image.load(f"{file_dir}/assets/textures/background-stars.png")
rockTexture = pygame.image.load(f"{file_dir}/assets/textures/rock_brown.png")
#spaceship animation
...
def initMainMenu():
    ...
    invisable_button = pygame.image.load(f"{file_dir}/assets/textures/invisable_button.png")       #Preload invisable button
    start_button = engine.properties_object("start_button", invisable_button, start_text.x, start_text.y, start_text.texture.get_width(), start_text.texture.get_height(), not performance_mode, 2)
    options_button = engine.properties_object("options_button", invisable_button, options_text.x, options_text.y, options_text.texture.get_width(), options_text.texture.get_height(), not performance_mode, 2)
    exit_button = engine.properties_object("exit_button", invisable_button, exit_text.x, exit_text.y, exit_text.texture.get_width(), exit_text.texture.get_height(), not performance_mode, 2)
...
def initOptionsMenu():
    ...
    #create boxes that are invisable
    invisable_button = pygame.image.load(f"{file_dir}/assets/textures/invisable_button.png")       #Preload invisable button
    back_button = engine.properties_object("back_button", invisable_button, back_text.x, back_text.y, back_text.texture.get_width(), back_text.texture.get_height(), not performance_mode, 2)
    volume_down_button = engine.properties_object("volume_down_button", invisable_button, volume_down_text.x, volume_down_text.y, volume_down_text.texture.get_width(), volume_down_text.texture.get_height(), not performance_mode, 2)
    volume_up_button = engine.properties_object("volume_up_button", invisable_button, volume_up_text.x, volume_up_text.y, volume_up_text.texture.get_width(), volume_up_text.texture.get_height(), not performance_mode, 2)
    performance_button = engine.properties_object("performance_button", invisable_button, performance_text.x, performance_text.y, performance_text.texture.get_width(), performance_text.texture.get_height(), not performance_mode, 2)
