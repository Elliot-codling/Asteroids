...
# Rocks
rockTex = engine.loadImage("assets/textures/rock_brown.png")
rockSpeed = 2

# Player
playerSpeed = 6
player = engine.spriteObject("player", "assets/textures/spaceship.png", engine.vector2f(0, 0), engine.vector2i(15 * 3, 17 * 3), not performanceMode, layer.PLAYER.value)

backgroundMusicTrack = [f"{engine.directoryPath()}/assets/music/menu_start.ogg", f"{engine.directoryPath()}/assets/music/gameplay.ogg"]
backgroundMusic = engine.music(backgroundMusicTrack, 0)


# === Move rocks ===
def moveRocks(window: engine.window, object: engine.spriteObject):
    global score
    ...
    object.incrementPosition(engine.vector2f(0, rockSpeed))
    

# === Scroll background ===
def scrollBackgroundObject(object: engine.spriteObject):
    if object.getID() != "bg":
        return
    
    if object.getPosition().y >= 600:
        object.setPosition(engine.vector2f(object.getPosition().x, -object.getSize().y))

    object.incrementPosition(engine.vector2f(0, 1))

# === Player movement ===
def playerMovement(window: engine.window):
    # Move the player
    if (window.getKey("a") or window.getKey("LEFT")) and player.leftBorder(player.getPosition(), 0):
        player.incrementPosition(engine.vector2f(-playerSpeed, 0))
    ...
    if not player.leftBorder(player.getPosition(), 0):
        player.setPosition(engine.vector2f(0, player.getPosition().y))
    if not player.rightBorder(player.getPosition() + player.getSize(), 480):
        player.setPosition(engine.vector2f(480 - player.getSize().x, player.getPosition().y))


# === Create rocks ===
def createRocks(window: engine.window):
    ...
    for x in range(0, 5):
        ...
        rock = engine.spriteObject("rock", rockTex, engine.vector2f((x * 95) + 10, -80), engine.vector2i(80, 80), not performanceMode, layer.ROCKS.value)
...
def start(window: engine.window) -> None:
    ...
    # Create background
    backgroundStarTex = engine.loadImage("assets/textures/background-stars.png")

    for y in range(11):
        for x in range(8):
            angle = random.choice([0, 90, 180, 270])

            backgroundStar = engine.spriteObject("bg", backgroundStarTex, engine.vector2f(x * 60, y * 60), engine.vector2i(60, 60), False, layer.BACKGROUND.value)
            backgroundStar.setAngle(angle)
            window.pushToQueue(backgroundStar)
    
    # Player
    player.setPosition(engine.vector2f((480 / 2) - (player.getSize().x / 2), 500))
...
def fixedUpdate(window: engine.window, deltaTime: float) -> None:
    ...
    if hitRock:
        engine.printDebugInfo(f"Game over, score: {score}")

    if window.getEvent("KEYDOWN") and window.getKey("f"):
        engine.printDebugInfo(window.getFramerate())
