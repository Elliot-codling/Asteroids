...
volume = 5             #volume of the game 
performance_mode = False                #performance mode - disables all transparencies
enable_vsync = True             #enables vsync

#create window -------------------------------------------
w, h = 480, 600
window = engine.window("Asterods", w, h, (0, 0, 0), 0, enable_vsync)

#load in textures - to always store them in ram
global background_stars_texture, rock_texture
background_stars_texture = pygame.image.load(f"{file_dir}/assets/textures/background-stars.png")
rock_texture = pygame.image.load(f"{file_dir}/assets/textures/rock_brown.png")
#display
display = []
...
display_sprite = []
#spaceship animation
playerAnimation = [pygame.image.load(f"{file_dir}/assets/textures/spaceship.png"), pygame.image.load(f"{file_dir}/assets/textures/spaceship_3.png"), pygame.image.load(f"{file_dir}/assets/textures/spaceship_2.png")]
#foreground
foreground = []
#text_foreground
text_foreground = []
...
def init_main_menu():               #creates all the text and all the invisable buttons
    ...
    start_button = engine.properties_object("start_button", f"{file_dir}/assets/textures/invisable_button.png", start_text.x, start_text.y, start_text.texture.get_width(), start_text.texture.get_height(), not performance_mode)
    options_button = engine.properties_object("options_button", f"{file_dir}/assets/textures/invisable_button.png", options_text.x, options_text.y, options_text.texture.get_width(), options_text.texture.get_height(), not performance_mode)
    exit_button = engine.properties_object("exit_button", f"{file_dir}/assets/textures/invisable_button.png", exit_text.x, exit_text.y, exit_text.texture.get_width(), exit_text.texture.get_height(), not performance_mode)
...
def init_options_menu():            #creates all the text and all the invisable buttons
    ...
    #create boxes that are invisable
    back_button = engine.properties_object("back_button", f"{file_dir}/assets/textures/invisable_button.png", back_text.x, back_text.y, back_text.texture.get_width(), back_text.texture.get_height(), not performance_mode)
    volume_down_button = engine.properties_object("volume_down_button", f"{file_dir}/assets/textures/invisable_button.png", volume_down_text.x, volume_down_text.y, volume_down_text.texture.get_width(), volume_down_text.texture.get_height(), not performance_mode)
    volume_up_button = engine.properties_object("volume_up_button", f"{file_dir}/assets/textures/invisable_button.png", volume_up_text.x, volume_up_text.y, volume_up_text.texture.get_width(), volume_up_text.texture.get_height(), not performance_mode)
    performance_button = engine.properties_object("performance_button", f"{file_dir}/assets/textures/invisable_button.png", performance_text.x, performance_text.y, performance_text.texture.get_width(), performance_text.texture.get_height(), not performance_mode)
...
def main_menu():            #pauses for 100ms to stop multiple clicks, main menu works by checking for mouse collisions with the invisable boxes created
    ...
    if not pygame.mouse.get_pressed()[0]:
        pass
    #main start menu
    elif engine.mouse.collision("start_button", foreground):
...
def reset_player():                 #resets the player
    ...
    rocket = engine.properties_object("rocket", f"{file_dir}/assets/textures/spaceship.png", (w/2) - 51 / 2, 518, 45, 51, not performance_mode)
...
def play_music():               #plays music depending if the code is in the main menu or in the main game
    if pygame.mixer.music.get_busy():
        ...
        if run_game == False:
            pygame.mixer.music.load(f"{file_dir}/assets/music/menu_start.ogg")
            pygame.mixer.music.play(-1)
        else:
            pygame.mixer.music.load(f"{file_dir}/assets/music/gameplay.ogg")
            pygame.mixer.music.play(-1)
